...
local function log(msg)
    DEFAULT_CHAT_FRAME:AddMessage("|cFF33FF22SA|r:" .. msg)
end
...
function SoundAlerter:OnInitialize()
    ...
    for _, v in pairs(self.spellList) do
        for _, spell in pairs(v) do
            if dbDefaults.profile[spell] == nil then
                dbDefaults.profile[spell] = true
            end
        end
    end
    self.db1 = LibStub("AceDB-3.0"):New("SoundAlerterDB", dbDefaults, "Default");
    DEFAULT_CHAT_FRAME:AddMessage("|cffFF7D0ASoundAlerter|r for TBC Classic by |cff0070DETrolollolol|r backported by Schaka");
...
function SoundAlerter:OnEnable()
    ...
    SoundAlerter:RegisterEvent("ARENA_COOLDOWNS_UPDATE")

    if not SA_LANGUAGE[sadb.path] then
        sadb.path = SA_LOCALEPATH[GetLocale()]
    end
        ...
        self.options.args.custom.args[key] = {
            ...
            args = {
                name = {
                    ...
                    set = function(info, value)
                        if sadb.custom[value] then
                            log(L["same name already exists"])
                            return
                        end
                        sadb.custom[key].name = value
                ...
                test = {
                    ...
                    func = function()
                        PlaySoundFile(sadb.custom[key].soundfilepath)
                    end,
                },
                soundfilepath = {
                    name = L["File Path"],
                ...
                eventtype = {
                    ...
                    values = SA_EVENT,
                    get = function(info, k)
                        return sadb.custom[key].eventtype[k]
                    end,
                    set = function(info, k, v)
                        sadb.custom[key].eventtype[k] = v
                    end,
                },
                sourceuidfilter = {
                    type = 'select',
                ...
                sourcecustomname = {
                    ...
                    disabled = function()
                        return not (sadb.custom[key].sourceuidfilter == "custom")
                    end,
                },
                destuidfilter = {
                    type = 'select',
                ...
                destcustomname = {
                    ...
                    name = L["Custom unit name"],
                    disabled = function()
                        return not (sadb.custom[key].destuidfilter == "custom")
                    end,
                },
                --[[NewLine5 = {
                    type = 'header',
...
function SoundAlerter:PlaySpell(list, spellID, ...)
    ...
    if not list or not list[spellID] then
        return
    end

    if not sadb[list[spellID]] then
        return
    end

    if sadb.throttle ~= 0 and self:Throttle("playspell", sadb.throttle) then
        return
    end

    if sadb.smartDisable then
        if (GetNumRaidMembers() or 0) > 20 then
            return
        end
        if self:Throttle("smarter", 20) then
            self.smarter = self.smarter + 1
            if self.smarter > 30 then
                return
            end
        else
            self.smarter = 0
...
function SoundAlerter:trinketDelay(spell, spellid)
    if spellid ~= nil and spell ~= nil then
        ...
        self:PlaySpell(self.spellList.castSuccess, 42292)
    end
end

function SoundAlerter:ARENA_COOLDOWNS_UPDATE(unit)
    local guid = UnitGUID(unit);
    local class, classFile = UnitClass(unit);
    local spellID, itemID, startTime, duration = C_PvP.GetArenaCrowdControlInfo(unit);

    if (spellID == 42219) then
        print("itemID " .. itemID .. " " .. startTime)
    end

    if sadb.trinketalert and not sadb.chatalerts and not sadb.caonlyTF then
...
function SoundAlerter:COMBAT_LOG_EVENT_UNFILTERED()
        ...
        for k in pairs(SA_UNIT) do
            if k == "party" then
                if UnitName("party1") ~= nil then
                    --because UnitInParty always returns true?
    ...
    if (event == "SPELL_AURA_APPLIED") then
        if spellID == 6770 or spellID == 11297 or spellID == 12826 --[[polymorph rank 4]] or spellID == 118 or spellID == 28271 or spellID == 28272 or spellID == 2094 or spellID == 33786 or spellID == 51514 or spellID == 5782 or spellID == 6215 then
            --sap/blind/cyclone/poly/fear chat & sound alerts
            if destName == playerName then
                if spellID == 33786 or spellID == 5782 or spellID == 6215 and not sadb.auraApplied then
                    -- Cyclone / Fear
                ...
                if not sadb.chatalerts then
                    ...
                    elseif (spellID == 12826 or spellID == 118 or spellID == 28271 or spellID == 28272) and sadb.polyalert then
                        -- poly
            ...
            elseif (desttype[COMBATLOG_FILTER_FRIENDLY_UNITS] and destName ~= playerName and ((destuid.target or destuid.focus) or (currentZoneType == "arena"))) then
                ...
                if not sadb.chatalerts then
                    if (spellID == 6770 or spellID == 11297) and sadb.sapalert then
                        --this worked by default, everything below didn't / replacing "(#friend#)" with destName did nothing either
        ...
        elseif not sadb.auraApplied then
            if destName == PlayerName then
                --Also includes buffs e.g "Cold Snap" - Should I leave this?
                self:PlaySpell(self.spellList.castSuccess, spellID)
                self:PlaySpell(self.spellList.castStart, spellID)
            elseif (desttype[COMBATLOG_FILTER_FRIENDLY_UNITS] and destName ~= playerName and (destuid.target or destuid.focus or currentZoneType == "arena")) then
                self:PlaySpell(self.spellList.friendCCSuccess, spellID)
    ...
    elseif (event == "SPELL_AURA_REMOVED" and not sadb.auraRemoved) then
        if destuid.target or destuid.focus or (destName == arena1 or destName == arena2 or destName == arena3 or destName == arena4 or destName == arena5) then
            --Can only hear sap down if it's your target/focus - source name returns nil
    ...
    elseif (event == "SPELL_CAST_START" and sourcetype[COMBATLOG_FILTER_HOSTILE_PLAYERS] and (not sadb.conlyTF or sourceuid.target or sourceuid.focus) and not sadb.castStart) then
        if currentZoneType == "arena" and (spellID == 33786 or spellID == 118 or spellID == 28272 or spellID == 28271 or spellID == 5782 or spellID == 6215) and not sadb.ArenaPartner and destName ~= playerName then
            -- fear, cyclone, poly
    ...
    elseif (event == "SPELL_CAST_SUCCESS" and sourcetype[COMBATLOG_FILTER_HOSTILE_PLAYERS]) or (event == "SPELL_CAST_SUCCESS" and spellID == 42292) then
        if ((spellID == 1784 or spellID == 1787 or spellID == 1856 or spellID == 642) and (not sadb.sonlyTF or sourceuid.target or sourceuid.focus) and not sadb.castSuccess) then
            --vanish/stealth chat alert
            self:PlaySpell(self.spellList.castSuccess, spellID)
        ...
        elseif (spellID == 42292) then
            --FIXME - no trinket combat log event??
        ...
        elseif spellID ~= 6770 and spellID ~= 11297 and spellID ~= 2094 and (not sadb.sonlyTF or sourceuid.target or sourceuid.focus) then
            --to prevent double spam in party
            self:PlaySpell(self.spellList.castSuccess, spellID)
        end
    ...
    for k, css in pairs(sadb.custom) do
        ...
        if css.eventtype[event] and destuid[css.destuidfilter] and desttype[css.desttypefilter] and sourceuid[css.sourceuidfilter] and sourcetype[css.sourcetypefilter] and spellID == tonumber(css.spellid) then
            if self:Throttle(tostring(spellID) .. css.name, 0.1) then
                return
            end
            if sadb.debugmode then
